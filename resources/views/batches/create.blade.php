...
<div class="container mx-auto px-6 py-8">
    <h3 class="text-gray-700 text-3xl font-medium">Add New Batch</h3>
    
    <div class="mt-8">
        <form action="{{ route('batches.store') }}" method="POST" class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4" id="batchForm">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="mb-4">
                    ...
                    <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('name') border-red-500 @enderror" id="name" type="text" name="name" value="{{ old('name') }}" placeholder="Batch 1" autofocus>
                ...
                <div class="mb-4">
                    ...
                    <select class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('course_id') border-red-500 @enderror" id="course_id" name="course_id">
                        ...
                        @foreach($courses as $course)
                            <option value="{{ $course->id }}" data-fee="{{ $course->default_total_fee }}" {{ old('course_id') == $course->id ? 'selected' : '' }}>{{ $course->name }}</option>
                        @endforeach
                    </select>
                ...
                <div class="mb-4">
                    ...
                    <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('start_date') border-red-500 @enderror" id="start_date" type="date" name="start_date" value="{{ old('start_date') }}">
                ...
                <div class="mb-4">
                    ...
                    <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('end_date') border-red-500 @enderror" id="end_date" type="date" name="end_date" value="{{ old('end_date') }}">
                ...
                <div class="mb-4">
                    ...
                    <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('total_fee') border-red-500 @enderror" id="total_fee" type="number" step="0.01" name="total_fee" value="{{ old('total_fee') }}" placeholder="Auto-filled from course">
                ...
                <div class="mb-4">
                    ...
                    <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('capacity') border-red-500 @enderror" id="capacity" type="number" name="capacity" value="{{ old('capacity') }}" placeholder="30">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-2">
                        @forelse($teachers as $teacher)
                            <label class="inline-flex items-center">
                                <input type="checkbox" name="teachers[]" value="{{ $teacher->id }}" class="form-checkbox h-5 w-5 text-red-600" {{ in_array($teacher->id, old('teachers', [])) ? 'checked' : '' }}>
                                <span class="ml-2 text-gray-700">{{ $teacher->name }}</span>
                            </label>
                        @empty
                            <p class="text-gray-500 col-span-4">No teachers available. Please create teacher users first.</p>
                        @endforelse
                    </div>
